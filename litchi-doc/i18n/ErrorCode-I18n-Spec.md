...
每个 ErrorCode 生成 **一个 key 插入 + 两条 message 插入（en-US + zh-CN）**，三条语句放在一起，每个 ErrorCode 前单独 `SET @ORDER_NUM = {序号}` 指定顺序值。
...
| `order_num` | 显示顺序 | 直接使用 `@ORDER_NUM` 指定的值 |
