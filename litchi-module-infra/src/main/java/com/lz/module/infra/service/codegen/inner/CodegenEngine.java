package com.lz.module.infra.service.codegen.inner;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 litchi-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 litchi-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.lz.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/litchi-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/litchi-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "litchi-module-${table.moduleName}/" + // 顶级模块
                "litchi-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "litchi-module-${table.moduleName}/" + // 顶级模块
                "litchi-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "litchi-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "litchi-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
